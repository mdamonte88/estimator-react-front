  ...
  clearBucket() {
    return new Promise((resolve, reject) => {
      this.client.listObjects({ Bucket: this.bucket }, (err, data) => {
        ...
        let deleteItemsPromises = [];
        items.forEach((item, index) => {
          let reg = /^uploads.*/gm;
          if (!reg.exec(item.Key)) {
            const deleteParams = { Bucket: this.bucket, Key: item.Key };
            deleteItemsPromises.push(this.client.deleteObject(deleteParams));
          }
        })
        Promise.all(deleteItemsPromises)
            .then((...data) => {
              resolve();
            })
            .catch(err => reject(`delete err ${deleteParams.Key}`))
      })
    })
  }
